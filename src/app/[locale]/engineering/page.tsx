        ...
        <ol className="mt-12 grid gap-6 md:grid-cols-3">
          {process.map((step, i) => (
            <Reveal as="li" key={step.title} delay={i * 80}>
              <div className="h-full rounded-lg border border-ink/10 bg-ink/[0.03] p-7">
          ...
          {capabilities.map((c, i) => (
            <Reveal key={c.title} delay={i * 60}>
              <div className="flex gap-5">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded border border-ink/10 bg-ink/[0.04] text-gold">
